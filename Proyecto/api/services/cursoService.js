...
async function crearCurso(body, idUsuario) {
  const { errores, datos } = validarDatosCurso(body);
  if (errores.length > 0) {
    return { errores, datos, nuevoId: null };
  }

  const nuevoId = await cursoRepository.crear(datos, idUsuario);
...
async function actualizarCurso(id, body, idUsuario) {
  const { errores, datos } = validarDatosCurso(body);
  if (errores.length > 0) {
    return { errores, datos, filas: null };
  }

  const filas = await cursoRepository.actualizar(id, datos, idUsuario);
  if (filas === 0) {
    throw createError(404, 'Curso no encontrado');
  }
...
async function eliminarCurso(id, idUsuario) {
  const filas = await cursoRepository.eliminar(id, idUsuario);
  if (filas === 0) {
    throw createError(404, 'Curso no encontrado');
  }
  return filas;
